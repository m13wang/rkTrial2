...
public class BluetoothChatService {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            char[] In_Buffer = new char[20];

            // Keep listening to the InputStream while connected and look for packets
            while (true) {
                try {
                    // Read from the InputStream
                    ThisByte = (char) mmInStream.read();
                    ...
                    if (DataPointer < 20)
                    {
                        In_Buffer[DataPointer] = ThisByte;
                        buffer[DataPointer] = (byte) ThisByte;
                        DataPointer++;
                    }

                    //got a full packet
                    if (DataPointer >= 10 && ThisByte == ';')
                    {
                        // Send the obtained bytes to the UI Activity
                        mHandler.obtainMessage(Main.MESSAGE_READ, 10, -1, buffer).sendToTarget();
                    }
